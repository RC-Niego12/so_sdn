...
$total_records_result = $conn->query("SELECT COUNT(*) AS total FROM tbl_save_clientbene INNER JOIN tbl_save_addl_entry
    ON tbl_save_clientbene.id_tbl_save_clientbene = tbl_save_addl_entry.id_tbl_save_addl_entry WHERE
...
$werswo_query = " AND swo_staffid = '".$_SESSION['staffid']."' ";
...
for ($i = 0; $i < count($columns); $i++) {
    if (isset($_POST['columns'][$i]['search']['value']) && $_POST['columns'][$i]['search']['value'] != '') {
        $search_query .= (empty($search_query) ? ' WHERE ' : ' AND ') . $columns[$i] . " LIKE '%" . $_POST['columns'][$i]['search']['value'] . "%'";
    }
}
...
$filtered_records_result = $conn->query(" SELECT COUNT(*) AS total FROM tbl_save_clientbene 
        INNER JOIN tbl_save_addl_entry 
        ON tbl_save_clientbene.id_tbl_save_clientbene = tbl_save_addl_entry.id_tbl_save_addl_entry " . $werswo_query . $search_query);
...
$data_query = "SELECT * FROM tbl_save_addl_entry INNER JOIN tbl_save_clientbene ON 
    ...
    " . $search_query . $werswo_query . $order_query . " LIMIT " . intval($limit) . " OFFSET " . intval($offset);
